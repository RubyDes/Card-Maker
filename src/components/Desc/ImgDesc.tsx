...
const DescInsertPic = (props: DispatchProps) => {
  const visibleDescInsertPic = useSelector(
    (state: RootState) => state.buttonsReducer.visibleDescInsertPic,
  );
  ...
  return (
    <div
      ...
      style={visibleDescInsertPic ? { display: "block" } : { display: "none" }}
    >
      <button
...
const mapDispatchToProps = (dispatch: Function) => {
  return {
    insertImg: (newSrc: string, width: number, height: number) =>
      dispatch(insertImg(newSrc, width, height)),
  };
};
