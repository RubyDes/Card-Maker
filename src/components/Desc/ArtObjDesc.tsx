...
const DescArtObj = (props: Props) => {
    ...
    return (
        <div className={styles.desc_art} style={visibleDescArtObj ? {display: "block"} : {display: "none"}}>
            <button className={styles.button} onClick={props.insertCircle}>Circle</button>
            <button className={styles.button} onClick={props.insertHeart}>Triangle</button>
        </div>
    )
}
